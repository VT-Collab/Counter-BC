...
# train counter-bc model
def train_model(args):

    # training parameters
    print("[-] training counter-bc")
    EPOCH = 1000
    LR = 0.001
    ...
    for epoch in range(EPOCH+1):
        for batch, x in enumerate(train_set):
            ...
            # start with bc over counterfactuals
            states = x[:, :, 0:4].to(device=args.device)
            actions = x[:, :, 4:6].to(device=args.device)
            log_pi = model.get_log_prob(states, actions)
            log_pi = log_pi.sum(dim=2)

            # get R(s, a) = pi(a \mid s) over counterfactuals
            pi_hat = torch.nn.functional.softmax(log_pi, dim=1)

            # loss is -R(s, a) * log_pi
            loss = torch.mean(-pi_hat * log_pi)
        ...
        if epoch % 500 == 0:
            ...
            torch.save(model.state_dict(), "models/counter-bc")
