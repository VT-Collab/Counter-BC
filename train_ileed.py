...
def train_model(args):
    ...
    for epoch in range(EPOCH+1):
        for batch, x in enumerate(train_set):
            ...
            # get R(s, a) = rho(s)
            rho = model.rho(states).squeeze(1)
            rho = torch.nn.functional.softmax(rho, dim=0)

            # loss is -R(s, a) * log_pi
            loss = torch.mean(-rho * log_pi)
        ...
        if epoch % 500 == 0:
            ...
            torch.save(model.state_dict(), "models/ileed")
